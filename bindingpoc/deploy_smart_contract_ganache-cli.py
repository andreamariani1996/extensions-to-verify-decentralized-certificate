...
file_accounts = open("keys.json", "r")
accounts_information = json.load(file_accounts)
file_accounts.close()

default_account_address = web3.eth.accounts[0]
default_account_address_lower = default_account_address.lower()

default_account_sk = "0x" + accounts_information["private_keys"][default_account_address_lower]
default_account_sk_bytes = decode_hex(default_account_sk)
default_account_sk_app = keys.PrivateKey(default_account_sk_bytes)
default_account_pk_bytes = default_account_sk_app.public_key
default_account_pk = default_account_pk_bytes.to_hex()[2:]
# l'indirizzo si può generare anche dalla chiave pubblica, è tutto legato
# default_account_address = default_account_pk_bytes.to_checksum_address()

print("l'indirizzo dell'account di default è: " + default_account_address)
print("la chiave privata dell'account di default è: " + default_account_sk)
print("la chiave pubblica dell'account di default è: " + default_account_pk)
...
environment["User"]["sK"] = default_account_sk
environment["User"]["pK"] = default_account_pk
